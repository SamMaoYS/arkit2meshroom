...
def configure(args):
    if not IO.file_exist(args.input, '.zlib'):
        IO.MULTISCAN_LOG(f'Input file {args.input} not exists', IO.ERR)
        return False

    IO.make_clean_folder(args.output)
    if not IO.folder_exist(args.output):
        IO.MULTISCAN_LOG(f'Create result folder {args.output} failed', IO.ERR)
        return False
    ...
    if args.width < 1:
        IO.MULTISCAN_LOG(f'Width of frames must be positive integers', IO.ERR)
        return False

    if args.height < 1:
        IO.MULTISCAN_LOG(f'Height of frames must be positive integers', IO.ERR)
        return False
...
def decompress(args, depth=True):
    ...
    IO.MULTISCAN_LOG('temp file name: '+tmp.name)
...
def write_frames(args):
    tmp = decompress(args)
    confidence_filter = args.input_confidence and IO.file_exist(args.input_confidence)
    if confidence_filter:
        tmp_confi = decompress(args, False)
    num_frames, frame_size = get_num_frames(args, tmp)
    out_num = int(num_frames/args.step)
    IO.MULTISCAN_LOG(f'{out_num} frames are being extracted')
    count = 0
